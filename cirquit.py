...
class Circuit(object):
	...
	def pure_state_tensor_to_density_matrix(self,pure_states, probs):
		...
		density_matrix = tf.reduce_sum(
				prod * tf.cast(tf.expand_dims(tf.expand_dims(probs, 1), 2), tf.complex64),
				0)
	...
	def create_circuits(self):
		...
		for l in range(2,1+self.L):
			...
			self.depolarization     = Add_Circuit(self.depolarization, append=depolarization)
	...
	def unitary_layer(self,layer):
		...
		if self.ansatz == "qaoa-r":
			...
			self.gamma_symbols.append(gamma)
			unitary  += cirq.Circuit((cirq.rz(gamma)(self.qubits[n]) for n in range(self.N)))
			gamma     = sympy.Symbol("Rx_L_{0}".format(layer))
			self.gamma_symbols.append(gamma)
			unitary  += cirq.Circuit((cirq.rx(gamma)(self.qubits[n]) for n in range(self.N)))
	...
	def depolarizing_layer(self,layer):
	...
	def gamma_init(self,loc=0):
		p_gammas            = tfp.distributions.Uniform(low=0.0001, high=0.05) # Kastoryano et al. (2020)
		gammas              = p_gammas.sample(sample_shape=(1,len(self.gamma_symbols)))
		return tf.Variable(gammas)


	def sample_errors(self, p_err=None, K=None):
		p_err               = self.p_err if p_err is None else p_err
		K                   = self.K if K is None else K
		p_errs              = np.tile(self.p_errs,(self.L*3)) if self.multilambda else np.array([(p_err/3) for _ in range( self.L*self.N*3)])  # multiply by 3 because X,Y,Z gate
		bern_dist           = tfp.distributions.Bernoulli(probs=p_errs, dtype=tf.float32)
		errors             	= tf.squeeze(bern_dist.sample(K))
		return errors

	def sample_errors_i(self, i, p_i):
		K                   = self.K
		p_dist 							= self.p_errs
		p_dist[i]						= p_i
	...
	def plot_history(self,save=False):
		...
		for i_c,col in enumerate(["F","G","H","S","p_err"]):
			...
			if col == "G" or col == "S":
				...
				if not self.multilambda:
					plt.plot(self.history[col+"_approx"],"--*",label=r"$\hat{"+col+"}$");
					min_ = np.minimum(np.min(self.history[col]),np.min(self.history[col+"_approx"]))
					max_ = np.maximum(np.max(self.history[col]),np.max(self.history[col+"_approx"]))
				if col == "G":
					plt.plot([self.G_target]*len(self.history[col].values),"black",linewidth=2,label="Target")
				if col == "S":
					plt.plot([self.S_target]*len(self.history[col].values),"black",linewidth=2,label="Target")
				plt.legend()
			elif col == "F":
				plt.plot(self.history[col],"--*",label=col + " ({:.2E})".format(np.max(self.history[col])));
				plt.plot(self.history["T"],"--*",label="T" + " ({:.2E})".format(np.min(self.history["T"])));
				plt.plot(self.history["RMSE"],"--*",label="RMSE" + " ({:.2E})".format(np.min(self.history["RMSE"])));
				min_ = 0
				max_ = 1
				plt.ylim([min_,max_])
				plt.legend()
				...
				plt.ylabel(col)
				if col == "H":
					plt.plot([self.H_target]*len(self.history[col].values),"black",linewidth=2,label="Target")
					plt.plot([self.H_0]*len(self.history[col].values),linestyle="--",color="grey",linewidth=1,label=r"$E_0$")
					plt.plot([self.H_1]*len(self.history[col].values),linestyle="--",color="lightgrey",linewidth=1,label=r"$E_1$")
					plt.legend()
		...
		plt.subplot(2, 3, 6)
		plt.plot(np.abs(self.history["grad_H_p_err"]),"--*",label=r"$|\nabla_{\lambda} H|$");
		plt.plot(np.abs(self.history["grad_TS_p_err"]),"--*",label=r"$|\beta^{-1}\nabla_{\lambda} S|$");
		plt.plot(self.history["ngrad_H_gamma"],"--*",label=r"$||\nabla_{\gamma} H||_2$");
		plt.xlabel("Epochs"); plt.legend()
		plt.ticklabel_format(axis='y', style='sci'); plt.yscale("log")

		if save:
			f.savefig(self.savepth+"history---"+self.settings+".pdf")
		plt.show()

	def plot_density_matrices(self,dm=[],save=False):
		...
		axes.flat[0].grid(False); axes.flat[0].set_xticks([]); axes.flat[0].set_yticks([]); 
		axes.flat[0].set_title("Circuit density");

		im  = axes.flat[1].imshow(target_rho, vmin=min_, vmax=max_, aspect="auto")
		axes.flat[1].grid(False); axes.flat[1].set_xticks([]); axes.flat[1].set_yticks([]); 
	...
	def run_circuit(self,gammas_=None):
		...
		if self.DMS:
			...
			rho           = simulator.simulate(self.unitary,cirq.ParamResolver({symbol: self.gammas.numpy()[0][i] for i,symbol in enumerate(self.gamma_symbols_str)})).final_density_matrix
			return tf.convert_to_tensor(rho)
		else:
			values        = tf.concat([tf.tile(gammas, tf.constant([self.K,1])), self.errors], 1)
