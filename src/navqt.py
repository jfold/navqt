...
class NAVQT(Circuit):
    ...
    def __init__(self, **kwargs):
        ...
        self.d = 2 ** self.N
        self.m = self.L
        self.unitary_optimizer = getattr(tf.optimizers, self.gamma_optim)(
            learning_rate=self.gamma_lr
        )
        self.noise_optimizer = (
            getattr(tf.optimizers, self.p_err_optim)(
                learning_rate=self.p_err_annealing_lr
            )
            if self.annealing
            else getattr(tf.optimizers, self.p_err_optim)(learning_rate=self.p_err_lr)
        )
        ...
        self.p_err = (
            np.random.uniform(low=1e-8, high=1e-1, size=(1,))
            if self.lambda_random
            else np.array([self.p_err])
        )
        self.p_errs = (
            np.random.uniform(low=1e-8, high=self.p_err, size=(self.N,))
            if self.lambda_random
            else np.array([self.p_err] * self.N).squeeze()
        )
        ...
        """ Save settings """
        self.settings = "N-{}--H-{}--L-{}--A-{}--K-{}--beta-{}--p-err-{:.2E}--g-lr-{:.2E}--l-lr-{:.2E}--cheat-{}--multi-{}--g-seed-{}--H-seed-{}".format(
            self.N,  ## p-{:.2E}--g-a-lr-{:.2E}--l-a-lr-{:.2E}--annealing-{}--
            self.model,
            self.L,
            self.ansatz,
            self.K,
    ...
    def __str__(self):
        ...
        str_ += (
            f"System: ".ljust(j)
            + f" N= {self.N}".ljust(j)
            + f"H= {self.model}".ljust(j)
            + f"beta= {self.beta}".ljust(j)
            + f"seed= {self.H_seed}  \n\r"
            + f"Tr[rho]= {self.tr_target_rho}  \n\r"
        )
        str_ += (
            f"Circuit: ".ljust(j)
            + f"Ansatz= {self.ansatz}".ljust(j)
            + f"L= {self.L}".ljust(j)
            + f"seed= {self.gamma_seed}  \n\r"
        )
    ...
    def get_H(self, loc=0, scale=1) -> cirq.PauliSum:
        ...
        if H_type == "IC" or H_type == "TFI":
            if coeff_type == "u":
                J_coeffs = np.ones(
                    (self.N,)
                )  # tf.linalg.normalize(, ord='euclidean')[0].numpy()
                h_z_coeffs = np.ones(
                    (self.N,)
                )  # tf.linalg.normalize(, ord='euclidean')[0].numpy()
                h_x_coeffs = np.ones(
                ...
                np.random.seed(self.H_seed)
                tf.random.set_seed(self.H_seed)
                J_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed
                )
                h_z_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed + 100
                )
                h_x_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed + 200
                )
            ...
            if coeff_type == "u":
                ...
                np.random.seed(self.H_seed)
                tf.random.set_seed(self.H_seed)
                J_XX_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed
                )
                J_YY_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed + 100
                )
                J_ZZ_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed + 200
                )
                h_X_coeffs = tfp.distributions.Normal(loc=loc, scale=scale).sample(
                    sample_shape=(self.N,), seed=self.H_seed + 1000
                )

                J_XX_coeffs = tf.linalg.normalize(J_XX_coeffs, ord="euclidean")[
                    0
                ].numpy()
                J_YY_coeffs = tf.linalg.normalize(J_YY_coeffs, ord="euclidean")[
                    0
                ].numpy()
                J_ZZ_coeffs = tf.linalg.normalize(J_ZZ_coeffs, ord="euclidean")[
                    0
                ].numpy()
                h_X_coeffs = tf.linalg.normalize(h_X_coeffs, ord="euclidean")[0].numpy()
        elif H_type == "RBM":
        ...
        if H_type == "IC":
            for i in range(self.N):
                if i < self.N - 1:
                    H -= (
                        float(J_coeffs[i])
                        * cirq.Z(self.qubits[i])
                        * cirq.Z(self.qubits[i + 1])
                    )
            ...
            for i in range(self.N - 1):
                if i < self.N - 1:
                    H -= (
                        float(J_XX_coeffs[i])
                        * cirq.X(self.qubits[i])
                        * cirq.X(self.qubits[i + 1])
                    )
                    H -= (
                        float(J_YY_coeffs[i])
                        * cirq.Y(self.qubits[i])
                        * cirq.Y(self.qubits[i + 1])
                    )
                    H -= (
                        float(J_ZZ_coeffs[i])
                        * cirq.Z(self.qubits[i])
            ...
            H -= (
                float(J_YY_coeffs[-1])
                * cirq.Y(self.qubits[-1])
                * cirq.Y(self.qubits[0])
            )
            H -= (
                float(J_ZZ_coeffs[-1])
                * cirq.Z(self.qubits[-1])
                * cirq.Z(self.qubits[0])
            )
    ...
    def H_expectation(self, p_err: float = None) -> tf.Tensor:
        if self.DMS:
            ...
            expectation = tfq.layers.Expectation(
                differentiator=tfq.differentiators.Adjoint(), backend=simulator
            )(
                self.unitary,
                operators=self.H,
            ...
            values = tf.concat(
                [tf.tile(self.gammas, tf.constant([self.K, 1])), self.errors], 1
            )
            expectation = tfq.layers.Expectation(
    ...
    def calc_targets(self) -> None:
        ...
        self.H_target = (
            tf.linalg.trace(tf.linalg.matmul(self.target_rho, self.H.matrix()))
            .numpy()
            .real
        )
    ...
    def error_gradient(
        ...
        for k, c in enumerate(coeffs):
            p_1 = np.maximum(
                np.minimum(self.ub, self.p_err + (k + 1) * self.epsilon), self.lb
            )
        ...
        if self.opt_true_G:  # CHEATING
            ...
            t2 = -np.log(inside)
            t3 = np.log(
                (1 - (1 - self.p_err) ** self.m + self.d * (1 - self.p_err) ** self.m)
                / self.d
            )
    ...
    def errors_gradient(self) -> tf.Tensor:
        ...
        for i_p, p_err in enumerate(self.p_errs):
            for k, c in enumerate(coeffs):
                p_1 = np.maximum(
                    np.minimum(self.ub, p_err + (k + 1) * self.epsilon), self.lb
                )
    ...
    def grid_search(self, n_evals: int = 100) -> None:
        ...
        for e in pbar:
            ...
            p_errors = tfp.distributions.Uniform(low=self.lb, high=1e-2).sample(
                sample_shape=(self.N)
            )
            p_errs = (
                tf.tile(p_errors, tf.constant([self.L * 3]))
                if self.multilambda
                else np.array([(p_errors[0] / 3) for _ in range(self.L * self.N * 3)])
            )
    ...
    def train(
        ...
        best_G_approxs = []
        pbar = tqdm(
            range(self.history.shape[0], self.history.shape[0] + n_epochs), leave=False
        )
        print("Training...")
        for e in pbar:
            # Gradients
            ...
            p_err_gradient = (
                self.errors_gradient() if self.multilambda else self.error_gradient()
            )
            if grad_norm:
                gamma_gradients /= tf.norm(gamma_gradients, ord="euclidean")
                p_err_gradient /= tf.norm(p_err_gradient, ord="euclidean")
            # Unitary update
            self.unitary_optimizer.apply_gradients(
                zip([gamma_gradients], [self.gammas])
            )
            # Noise update
            if self.multilambda:
                self.noise_optimizer.apply_gradients(
                    zip([p_err_gradient], [self.p_errs_tf])
                )
                self.p_errs = np.minimum(
                    self.ub, np.maximum(self.p_errs_tf.numpy(), self.lb)
                )
                self.p_errs_tf = tf.Variable(self.p_errs, dtype=tf.float32)
            else:
                self.noise_optimizer.apply_gradients(
                    zip([p_err_gradient], [self.p_err_tf])
                )
                self.p_err = np.minimum(
                    self.ub, np.maximum(self.p_err_tf.numpy(), self.lb)
                )
                self.p_err_tf = tf.Variable(self.p_err, dtype=tf.float32)
            ...
            if not np.isnan(cur_F) and cur_F > best_F:
                ...
                if (
                    early_stop
                    and not self.annealing
                    and len(best_Fs) > 100
                    and len(best_G_approxs) > 100
                ):
                    print("Terminated with convergence on F and G_approx")
                    break
                elif self.annealing and len(best_Fs) > 20:  # annealing switch
                    self.unitary_optimizer = getattr(tf.optimizers, self.gamma_optim)(
                        learning_rate=self.gamma_annealing_lr
                    )
                    self.noise_optimizer = getattr(tf.optimizers, self.p_err_optim)(
                        learning_rate=self.p_err_lr
                    )
                    best_Fs = [best_F]
            ...
            if self.p_err_lr_decay and e % 200 == 0:
                self.noise_optimizer.learning_rate = (
                    self.noise_optimizer.learning_rate * 0.1
                )
